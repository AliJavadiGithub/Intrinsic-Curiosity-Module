    ...
    while episode < max_steps:
        ...
        while not done:
            ...
            if ep_steps % T_MAX == 0 or done:
                ...
                loss = local_agent.calc_cost(obs, hx, done, rewards,
                                             values, log_probs, intrinsic_reward)
                optimizer.zero_grad()
                hx = hx.detach_()
                if icm:
                    global_icm_optim.zero_grad()
                    (L_I + L_F).backward()
        ...
        if name == '1':
            ...
            print('ICM episode {} thread {} of {} steps {:.2f}M score {:.2f}'
                    'avg score (100) (5000) {:.2f} {:.2f}'.format(episode, name, n_threads,
                                                    t_steps/1e6, score,
                                                    avg_score, avg_score_5000))

        if name == '1':
            steps_list = [z for z in range(episode)]
            plot_learning_curve(steps_list, scores, 'ICM_final.png')
